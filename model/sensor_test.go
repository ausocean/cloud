...
func TestTransform(t *testing.T) {
	...
	tests := []struct {
		...
		{sensor: Sensor{Func: string(keyScale), Args: "0.5"}, val: 20.0, want: 10.0},
		{sensor: Sensor{Func: string(keyScale), Args: "0.5 "}, val: 20.0, want: 10.0},
		{sensor: Sensor{Func: string(keyScale), Args: "0.5,9.5"}, err: ErrUnexpectedArgs},
		{sensor: Sensor{Func: string(keyScale), Args: "num"}, err: strconv.ErrSyntax},
		{sensor: Sensor{Func: string(keyLinear), Args: "2.0,3.0"}, val: 10.0, want: 23.0},
		{sensor: Sensor{Func: string(keyLinear), Args: "2.0 ,3.0 "}, val: 10.0, want: 23.0},
		{sensor: Sensor{Func: string(keyLinear), Args: "2.0,3.0,4.0"}, err: ErrUnexpectedArgs},
		{sensor: Sensor{Func: string(keyLinear), Args: "2.0,num"}, err: strconv.ErrSyntax},
		{sensor: Sensor{Func: string(keyQuadratic), Args: "2.0,3.0,4.0"}, val: 2.0, want: 18.0},
		{sensor: Sensor{Func: string(keyQuadratic), Args: "2.0,3.0 ,4.0"}, val: 2.0, want: 18.0},
		{sensor: Sensor{Func: string(keyQuadratic), Args: "2.0,3.0"}, err: ErrUnexpectedArgs},
		{sensor: Sensor{Func: string(keyQuadratic), Args: "2.0,3.0,num"}, err: strconv.ErrSyntax},
		{sensor: Sensor{Func: string(keyCustom), Args: "(x+10.0)/2.0"}, val: 20.0, want: 15.0},
		{sensor: Sensor{Func: string(keyCustom), Args: "+_FHK+-="}, err: ErrEvaluableExpression},
		{sensor: Sensor{Func: string(keyCustom), Args: "asdfasdf"}, err: ErrEvaluate},
	}
