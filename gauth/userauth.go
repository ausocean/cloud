...
type UserAuth struct {
	...
	store     backend.SessionStore // Session state.
}
...
func (ua *UserAuth) Init(h backend.HTTPHandler) {
	ua.Lock()
	defer ua.Unlock()
	...
	ua.store, err = h.NewSessionStore(secrets["sessionKey"])
	if err != nil {
		log.Printf("error creating new SessionStore: %v", err)
	}
...
func (ua *UserAuth) LoginHandler(h backend.HTTPHandler) error {
	ua.Lock()
	defer ua.Unlock()
	...
	oauthFlowSession, err := ua.store.Get(sessID, h.SessionStoreOptions()...)
	if err != nil {
	...
	err = ua.store.Save(oauthFlowSession, h.SessionStoreOptions()...)
	if err != nil {
...
func (ua *UserAuth) CallbackHandler(h backend.HTTPHandler) error {
	ua.Lock()
	defer ua.Unlock()
	...
	oauthFlowSession, err := ua.store.Get(h.FormValue("state"), h.SessionStoreOptions()...)
	if err != nil {
		return fmt.Errorf("could not get state parameter from session store: %w", err)
	}

	url, err := oauthFlowSession.Get(oauthFlowRedirectKey)
	redirectURL, ok := url.(string)
	if !ok || err != nil {
	...
	sess, err := ua.store.Get(ua.SessionID, h.SessionStoreOptions()...)
	if err != nil {
	...
	err = ua.store.Save(sess, h.SessionStoreOptions()...)
	if err != nil {
...
func (ua *UserAuth) LogoutHandler(h backend.HTTPHandler) error {
	ua.Lock()
	defer ua.Unlock()
...
func (ua *UserAuth) GetProfile(h backend.HTTPHandler) (*Profile, error) {
	ua.Lock()
	defer ua.Unlock()
	...
	v, err := sess.Get(oauthTokenSessionKey)
	if err != nil {
...
func (ua *UserAuth) PutData(h backend.HTTPHandler, data string) error {
	ua.Lock()
	defer ua.Unlock()
	...
	return ua.store.Save(sess, h.SessionStoreOptions()...)
}
