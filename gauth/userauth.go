...
type UserAuth struct {
	...
	store     sessions.Store // Session state.
}
...
func (ua *UserAuth) Init() {
	...
	ua.store = sessions.NewCookieStore([]byte(secrets["sessionKey"]))
}
...
func (ua *UserAuth) LoginHandler(w http.ResponseWriter, r *http.Request) error {
	...
	oauthFlowSession, err := ua.store.New(r, sessID)
	if err != nil {
		return fmt.Errorf("could not create session %s: %w", sessID, err)
	}
	oauthFlowSession.Options.MaxAge = ua.MaxAge
...
func (ua *UserAuth) CallbackHandler(w http.ResponseWriter, r *http.Request) error {
	...
	oauthFlowSession, err := ua.store.Get(r, r.FormValue("state"))
	if err != nil {
	...
	sess, err := ua.store.New(r, ua.SessionID)
	if err != nil {
	...
	sess.Values[oauthTokenSessionKey] = tok
	sess.Values[profileKey] = profile
	err = sess.Save(r, w)
	if err != nil {
...
func (ua *UserAuth) LogoutHandler(w http.ResponseWriter, r *http.Request) error {
	...
	sess.Options.MaxAge = -1 // Clear session.
	err = sess.Save(r, w)
	if err != nil {
		return fmt.Errorf("could not save session %s: %w", ua.SessionID, err)
	}
...
func (ua *UserAuth) GetProfile(w http.ResponseWriter, r *http.Request) (*Profile, error) {
	...
	sess, err := ua.store.Get(r, ua.SessionID)
	if err != nil {
		return nil, SessionNotFound
	}
	tok, ok := sess.Values[oauthTokenSessionKey].(*oauth2.Token)
	if !ok {
		return nil, TokenNotFound
	}

	profile, ok := sess.Values[profileKey].(*Profile)
	if !ok {
	...
	sess.Values[oauthTokenSessionKey] = newTok
	sess.Values[profileKey] = profile
