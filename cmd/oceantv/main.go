...
func broadcastHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = manager.NewOceanBroadcast(nil, &cfg, store, log, setVar, broadcastByName).Save(ctx, func(_cfg *Cfg) {
		...
		_cfg.AuthKeyVar = cfg.AuthKeyVar
		_cfg.StorageConfigVar = cfg.StorageConfigVar
		_cfg.CameraOutputVar = cfg.CameraOutputVar
		_cfg.BroadcastHost = cfg.BroadcastHost
		_cfg.VidforwardHost = cfg.VidforwardHost

		// The storage bucket and provider are user-configurable, but the
		// prefix and AuthKey are generated by oceantv when a broadcast is
		// created, so these must be preserved rather than overwritten.
		if cfg.StorageConfig != nil {
			if _cfg.StorageConfig == nil {
				_cfg.StorageConfig = &broadcast.StorageConfig{}
			}
			_cfg.StorageConfig.Bucket = cfg.StorageConfig.Bucket
			_cfg.StorageConfig.Provider = cfg.StorageConfig.Provider
		}
