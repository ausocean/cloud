...
async function fetchJSON<T>(
  ...
  try {
    ...
    if (e.name === "AbortError" || String(e).includes("aborted")) {
      throw new Error(
        `The request timed out after ${Math.round(ms / 1000)} seconds. This usually means the server was too busy processing the data. Adjust chunk sizes or try again later.`,
      );
    }
    throw e;
  } finally {
...
export class MediaManager extends TailwindElement() {
    ...
    try {
      ...
      if (this.selectedMonth) {
        ...
        if (this.selectedDay) {
          ...
          params.set(
            "from",
            String(new Date(year, month - 1, day).getTime() / 1000),
          );
          params.set(
            "to",
            String(new Date(year, month - 1, day + 1).getTime() / 1000),
          );
        } else {
          // local time start of month
          params.set(
            "from",
            String(new Date(year, month - 1, 1).getTime() / 1000),
          );
  ...
  private getAvailablePins(): string[] {
    ...
    for (const dev of this.devices) {
      if (this.selectedDeviceMac && String(dev.Mac) !== this.selectedDeviceMac)
        continue;
  ...
  render() {
    ...
    return html`
      <div class="max-w-screen-md mx-auto px-4 py-8 space-y-6">
        <div class="flex flex-wrap items-center gap-6">
          ...
          <div class="flex items-center gap-2">
            ...
            >
              ...
              ${this.devices.map(
                (d) => html`<option value="${d.Mac}">${d.Name}</option>`,
              )}
            </select>
          </div>
        ...
        <div
          class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 flex flex-col items-center justify-center space-y-4"
        >
          ${this.renderContent()}
        </div>
      </div>
  ...
  private renderBanner() {
    if (this.uiState === "error") {
      return html`
        <div
          class="flex items-center gap-3 rounded-lg border border-red-300 bg-red-50 dark:bg-red-900/20 dark:border-red-700 px-4 py-3 text-sm text-red-700 dark:text-red-300"
        >
          <svg class="w-5 h-5 shrink-0" fill="currentColor" viewBox="0 0 20 20">
            <path
              fill-rule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm-.75-11.25a.75.75 0 011.5 0v4a.75.75 0 01-1.5 0v-4zm.75 7a1 1 0 110-2 1 1 0 010 2z"
              clip-rule="evenodd"
            />
          </svg>
  ...
  private renderContent() {
    if (this.uiState === "loading") {
      return html`
        <div
          class="animate-pulse flex items-center gap-3 text-gray-500 dark:text-gray-400"
        >
    ...
    if (this.uiState === "deleting") {
      ...
      return html`
        <div class="text-center w-full max-w-sm">
          ...
          <div class="text-xs text-red-500 mt-2 font-medium animate-pulse">
    ...
    const count = Object.values(this.keysByDate).reduce(
    ...
    const allSelected =
      dates.length > 0 && dates.every((d) => this.selectedDates.has(d));
      ...
      ${count > 0
        ? html`
            <div class="w-full max-w-sm mt-4 text-left">
              <div
                class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 pb-1 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center"
              >
                <span>Clips by Day</span>
                <label
                  class="flex items-center gap-1.5 cursor-pointer text-gray-600 dark:text-gray-300"
                >
                  <input
                    type="checkbox"
                    .checked=${allSelected}
                    @change=${toggleAll}
                    class="rounded text-blue-600 focus:ring-blue-500 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600"
                  />
                  ...
                  (d) => html`
                    <div class="flex justify-between items-center text-sm py-1">
                      <label class="flex items-center gap-2 cursor-pointer">
                        <input
                          type="checkbox"
                          .checked=${this.selectedDates.has(d)}
                          @change=${(e: Event) => toggleDate(d, e)}
                          class="rounded text-blue-600 focus:ring-blue-500 bg-gray-100 border-gray-300 dark:bg-gray-700 dark:border-gray-600"
                        />
                        <span
                      ...
                      <span
                        class="text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-700/50 px-2 py-0.5 rounded-full text-xs"
                        >${this.summary[d]} clips</span
                      >
                    </div>
                  `,
                )}
            ...
            ${Number(this.selectedLimit) > 0 &&
            count >= Number(this.selectedLimit)
              ? html`
                  <div
                    class="mt-4 text-xs text-gray-500 dark:text-gray-400 max-w-sm"
                  >
                    Note: To preserve performance a load limit is applied, only
                    ${this.selectedLimit} clips are loaded at once. Delete this
                    batch to reveal older clips.
                  </div>
                `
              : nothing}
          `
  ...
  private renderModal() {
    ...
    return html`
      ...
      >
        <div
          class="w-full max-w-md rounded-xl bg-white dark:bg-gray-800 shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden pointer-events-auto"
        >
          <div
            class="flex items-center gap-3 px-6 py-4 border-b border-gray-200 dark:border-gray-700"
          >
            <span
              class="flex-shrink-0 rounded-full bg-red-100 dark:bg-red-900/30 p-2"
            >
              <svg
          ...
          <div class="px-6 py-4 space-y-3">
            <p class="text-sm text-gray-700 dark:text-gray-300">
              ...
              <span class="font-semibold text-red-600 dark:text-red-400"
                >${count} media clip${count === 1 ? "" : "s"}</span
              >
              from ${this.selectedDates.size} selected
              day${this.selectedDates.size === 1 ? "" : "s"}.
            </p>
          ...
          <div
            class="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/40"
          >
            <button
              @click=${() => {
                this.showDeleteModal = false;
