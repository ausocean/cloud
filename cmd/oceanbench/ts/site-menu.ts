...
class SiteMenu extends LitElement {
  ...
  async loadSites() {
    ...
    r.onreadystatechange = () => {
      if (r.readyState == XMLHttpRequest.DONE) {
        ...
        this.dispatchEvent(
          new CustomEvent("sites-loaded", {
            bubbles: true,
            composed: true,
            detail: { len: sites.length },
          }),
        );
  ...
  handleSiteChange(event: Event) {
    ...
    if (this.customHandling) {
      this.dispatchEvent(
        new CustomEvent("site-change", {
          bubbles: true,
          detail: {
            previousSite: this.selectedData,
            newSite: selectedKey + ":" + selectedName,
          },
        }),
      );
    ...
    if (selectedOpt.slot != this.selectedPerm) {
      ...
      this.dispatchEvent(
        new CustomEvent("permission-change", {
          bubbles: true,
          composed: true,
          detail: { selectedPerm: this.selectedPerm },
        }),
      );
    }
  }
