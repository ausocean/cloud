  ...
  private renderList(kind: ListKind) {
    ...
    const header = html`
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <span class="relative group">
            <svg
              ...
              class="w-4 h-4 text-gray-400 cursor-default"
              fill="currentColor"
              viewBox="0 0 24 24"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M12 2C6.477 2 2 6.477 2 12s4.477 10 10 10 10-4.477 10-10S17.523 2 12 2zm0 4a1.25 1.25 0 1 1 0 2.5A1.25 1.25 0 0 1 12 6zm-1 4h2v8h-2v-8z"
              />
            </svg>
            <span
              class="absolute left-1/2 -translate-x-1/2 bottom-full mb-2 w-56 rounded bg-gray-900 dark:bg-gray-700 px-3 py-2 text-xs text-white opacity-0 pointer-events-none group-hover:opacity-100 transition-opacity z-10 shadow-lg"
            >
              ${meta.info}
            </span>
          </span>
        ...
        ${items && !loading
          ? html`<span
              class="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200"
              >${items.length}</span
            >`
          : nothing}
      </div>
    ...
    if (loading) {
      ...
      body = html`<div class="px-4 py-6 text-sm text-gray-400">
        No sites found.
      </div>`;
    } else {
            ...
            (s) => html`
              <li class="flex items-center justify-between px-4 py-2.5">
                ...
                <div class="flex items-center gap-1.5">
                  ${s.Public
                    ? html`<span
                        class="text-xs font-medium px-2 py-0.5 rounded-full bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300"
                        >Public</span
                      >`
                    : nothing}
                  ${s.Perm !== undefined && s.Perm !== 0
                    ? html`<span
                        class="text-xs font-medium px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900 dark:text-blue-300"
                        >Perm: ${s.Perm}</span
                      >`
                    : nothing}
                </div>
    ...
    return html`
      <div class="max-w-7xl mx-auto px-4 py-6">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ${(Object.keys(listMeta) as ListKind[]).map((k) =>
            this.renderList(k),
          )}
        </div>
      </div>
