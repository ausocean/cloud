...
export class SiteNotification extends TailwindElement() {
  ...
  async loadNotification() {
    try {
      const res = await fetch("/temp/notification-002.json");
  ...
  render() {
    ...
    return html`
      <div class="relative mx-auto mt-4 max-w-3xl rounded-xl border-l-4 border-yellow-400 bg-yellow-900 bg-opacity-80 p-4 text-yellow-100 shadow-xl">
        <button class="absolute right-3 top-2 cursor-pointer text-lg text-yellow-300 hover:text-white" @click=${this.dismiss}>×</button>
