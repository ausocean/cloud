...
func main() {
	...
	if standalone {
		http.Handle("/src/", http.StripPrefix("/src/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			target := "http://localhost:5173/src/" + strings.TrimPrefix(r.URL.Path, "/src/")
			http.Redirect(w, r, target, http.StatusTemporaryRedirect)
		})))
	} else {
		http.Handle("/src/", http.StripPrefix("/src/", http.FileServer(http.Dir("webapp/dist/src"))))
	}
...
func setup(ctx context.Context) {
	...
	templateDir := "cmd/oceanbench/webapp/dist/"
