...
The failure mode toggle indicates that an issue has occurred whilst trying to start a broadcast. The issue must have prevented the broadcast from starting multiple times, as OceanTV allows for 3 retries before entering failure mode.

## Fixing the Fault

Failure mode should only be entered by the broadcast if something has actually gone wrong in the broadcast starting. This should pick up configuration errors, faulty connections, flat batteries, and dead hardware. If a broadcast has gone into failure mode, start by trying to identify the fault, and fix it before removing the broadcast from failure mode. Broadcasts can end up in failure mode due to timing issues occasionally so if no fault can be found, the broadcast can be removed from failure mode.
