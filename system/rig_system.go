...
// addWifi adds the wifi name and password to the rig system Controller.
// This implements the wifiHolder interface.
func (sys *RigSystem) AddWifi(ssid, pass string) {
	sys.Controller.Wifi = fmt.Sprintf("%s,%s", ssid, pass)
}

// addLocation adds the location of the system to the system controller,
// as well as any defined peripherals.
func (sys *RigSystem) AddLocation(lat, long float64) {
